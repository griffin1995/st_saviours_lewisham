...
export default function News() {
  ...
  const handleArticleShare = useCallback(
  ...
  const filteredArticles = newsArticles.filter((article) => {
    ...
    const matchesWatchedSearch =
      watchedValues.searchTerm === "" ||
      article.title
        .toLowerCase()
        .includes(watchedValues.searchTerm.toLowerCase()) ||
      article.excerpt
        .toLowerCase()
        .includes(watchedValues.searchTerm.toLowerCase());
    const matchesWatchedCategory =
      watchedValues.category === "All" ||
      article.category === watchedValues.category;
    const matchesAuthor =
      watchedValues.author === "all" ||
      (article as any).author === watchedValues.author;
    const matchesDateRange =
      watchedValues.dateRange === "all" ||
      checkDateRange(article.date, watchedValues.dateRange);
    const matchesTags =
      watchedValues.tags.length === 0 ||
      watchedValues.tags.some((tag: string) =>
        (article as any).tags?.includes(tag)
      );

    return (
      matchesSearch &&
      matchesCategory &&
      matchesWatchedSearch &&
      matchesWatchedCategory &&
      matchesAuthor &&
      matchesDateRange &&
      matchesTags
    );
    ...
    switch (sortBy) {
      ...
      case "popularity":
        ...
        return (
          ...
          bStats.likes +
          bStats.shares -
          (aStats.views + aStats.likes + aStats.shares)
        );
      case "title":
  ...
  const articleEngagementData = {
    labels: sortedArticles
      .slice(0, 10)
      .map((article) => article.title.substring(0, 20) + ".."),
    datasets: [
      {
        label: "Views",
        data: sortedArticles
          .slice(0, 10)
          .map((article) => articleStats[article.id]?.views || 0),
      ...
      },
      {
        ...
        data: sortedArticles
          .slice(0, 10)
          .map((article) => articleStats[article.id]?.likes || 0),
        ...
        <Container size="lg">
          <ScrollRevealSection>
            <div className="text-center mb-12">
              ...
              <p
                className={`${typographyScale.bodyLarge} text-gray-300 max-w-3xl mx-auto`}
              >
                Let Scripture guide our understanding of current events and
                community life
              </p>
            </div>
          ...
          >
            <div className="text-center mb-8">
              ...
              <p
                className={`${typographyScale.bodyLarge} text-gray-300 max-w-3xl mx-auto`}
              >
                Use our advanced search to find articles by topic, author, date,
                or tags
              </p>
            </div>
                  ...
                  <div className="grid lg:grid-cols-4 gap-4">
                    ...
                    <div className="relative">
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          setSortBy(
                            e.target.value as "date" | "popularity" | "title"
                          )
                        }
                        className="w-full pl-10 pr-4 py-3 border border-slate-500 rounded-lg focus:ring-2 focus:ring-gold-500 focus:border-transparent bg-white/20 backdrop-blur-sm text-white appearance-none"
                      >
                    ...
                    <div className="relative">
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          setViewMode(
                            e.target.value as "grid" | "list" | "timeline"
                          )
                        }
                        className="w-full pl-10 pr-4 py-3 border border-slate-500 rounded-lg focus:ring-2 focus:ring-gold-500 focus:border-transparent bg-white/20 backdrop-blur-sm text-white appearance-none"
                      >
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {tags.map((tag) => (
                        <label
                          key={tag}
                          className="flex items-center space-x-2 bg-white/10 px-3 py-2 rounded-lg hover:bg-white/20 transition-colors cursor-pointer"
                        >
                          <input
                            type="checkbox"
                            value={tag}
                  ...
                  <div className="flex items-center justify-between pt-4 border-t border-slate-600">
                    <div className="flex items-center gap-4">
                      ...
                      <label className="flex items-center space-x-2">
                        <input
                          ...
                          onChange={(e) =>
                            setIsInfiniteScrollEnabled(e.target.checked)
                          }
                          className="h-4 w-4 text-gold-600 focus:ring-gold-500 border-gray-300 rounded bg-white/20"
                        />
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Button
                        ...
                        onClick={() =>
                          window.open("/bookmarked-articles", "_blank")
                        }
                      >
            ...
            <ScrollRevealSection>
              ...
              <div className="grid lg:grid-cols-3 gap-8 mb-12">
                <Card
                  variant="default"
                  padding="lg"
                  className="bg-white/10 backdrop-blur-sm border border-slate-600 text-center"
                >
                  <CardContent>
                ...
                >
                  <CardContent>
                    ...
                    <h3 className={`${typographyScale.h3} text-white mb-2`}>
                      {Object.values(articleStats).reduce(
              ...
              >
                <CardContent>
                  <h3
                    className={`${typographyScale.h3} text-white mb-6 text-center`}
                  >
              ...
              <div className="relative">
                ...
                <div className="absolute top-4 right-4 flex items-center gap-2">
                  ...
                  <m.button
                    ...
                    whileTap={{ scale: 0.95 }}
                    onClick={() =>
                      handleArticleLike(String(sortedArticles[0].id))
                    }
                    className="p-2 rounded-full bg-white/20 text-gray-300 hover:bg-white/30 transition-colors"
                  >
            ...
            {displayedArticles.length > 0 ? (
              <div className="space-y-8">
                ...
                <div className="flex justify-center mb-8">
                  <div className="flex items-center gap-2 bg-white/10 backdrop-blur-sm rounded-xl p-2">
                    {[
                      { key: "grid", icon: DocumentTextIcon },
                      { key: "list", icon: ClockIcon },
                      { key: "timeline", icon: CalendarDaysIcon },
                    ].map(({ key, icon: Icon }) => (
                      <m.button
                        key={key}
                        whileHover={{ scale: 1.05 }}
                        whileTap={{ scale: 0.95 }}
                        onClick={() =>
                          setViewMode(key as "grid" | "list" | "timeline")
                        }
                  ...
                  <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {displayedArticles.slice(1).map((article, index) => (
                      ...
                      >
                        <Card
                          variant="default"
                          className="bg-white/10 backdrop-blur-sm border border-slate-600 hover:border-gold-300 hover:shadow-2xl transition-all duration-300 h-full"
                        >
                          <CardContent className="p-6">
                            ...
                            <div className="space-y-3">
                              <div className="flex items-center gap-2 text-xs text-gray-400">
                                ...
                                <span>
                                  {(article as any).author || "Parish News"}
                                </span>
                              </div>

                              <h3
                                className={`${typographyScale.h5} text-white group-hover:text-gold-300 transition-colors duration-300 line-clamp-2`}
                              >
                                {article.title}
                              </h3>

                              <p
                                className={`${typographyScale.body} text-gray-300 line-clamp-3`}
                              >
                                {article.excerpt}
                              </p>
                                  ...
                                  <div className="flex flex-wrap gap-1 mt-3">
                                    {(article as any).tags
                                      ...
                                      .map((tag: string) => (
                                        <span
                                          key={tag}
                                          className="px-2 py-1 bg-gold-600/20 text-gold-300 rounded-md text-xs"
                                        >
                                          {tag}
                                        </span>
                                      ))}
                  ...
                  <div className="space-y-4">
                    {displayedArticles.slice(1).map((article, index) => (
                      ...
                      >
                        <Card
                          variant="default"
                          className="bg-white/10 backdrop-blur-sm border border-slate-600 hover:border-gold-300 hover:shadow-xl transition-all duration-300"
                        >
                          <CardContent className="p-6">
                            <div className="flex items-start gap-6">
                              ...
                              <div className="flex-1 space-y-2">
                                ...
                                <h3
                                  className={`${typographyScale.h5} text-white group-hover:text-gold-300 transition-colors duration-300`}
                                >
                                  {article.title}
                                </h3>

                                <p
                                  className={`${typographyScale.body} text-gray-300 line-clamp-2`}
                                >
                                  {article.excerpt}
                                </p>
                ...
                {isInfiniteScrollEnabled &&
                  currentPage * articlesPerPage < sortedArticles.length && (
              ...
              <ScrollRevealSection>
                <Card
                  variant="default"
                  padding="lg"
                  className="text-center bg-white/10 backdrop-blur-sm border border-slate-600"
                >
                  <CardContent>
                    <div className="py-12">
                      ...
                      <p
                        className={`${typographyScale.body} text-gray-300 mb-6 max-w-md mx-auto`}
                      >
                  ...
                  <CardContent>
                    <div className="space-y-6">
                      <div className="text-center lg:text-left">
                        ...
                        <p
                          className={`${typographyScale.bodyLarge} text-gray-300`}
                        >
                          Join our parish newsletter to receive weekly updates,
                          spiritual reflections, and exclusive content delivered
                          to your inbox.
                        </p>
                      </div>
                    ...
                    {[
                      {
                        ...
                        title: "Weekly Parish News",
                        description: "Latest updates from our community",
                      },
                      {
                        icon: CalendarDaysIcon,
                        title: "Upcoming Events",
                        description: "Never miss important parish activities",
                      },
                      {
                        icon: BookmarkIcon,
                        title: "Spiritual Reflections",
                        description: "Weekly thoughts and prayers",
                      },
                      {
                        icon: HeartIcon,
                        title: "Community Stories",
                        description: "Inspiring tales from our parish family",
                      },
                    ].map((benefit, index) => (
                      <m.div
                        key={benefit.title}
                      ...
                      >
                        ...
                        <div>
                          <h4
                            className={`${typographyScale.h5} text-white mb-2`}
                          >
                            {benefit.title}
                          </h4>
                          <p
                            className={`${typographyScale.body} text-gray-300`}
                          >
                            {benefit.description}
                          </p>
                        </div>
          ...
          <ScrollRevealSection>
            <div className="text-center mb-12">
              ...
              <p
                className={`${typographyScale.bodyLarge} text-gray-300 max-w-3xl mx-auto`}
              >
                Discover additional ways to stay connected with our parish
                community
              </p>
            </div>
                ...
                >
                  ...
                  >
                    <CardContent className="p-6 text-center">
                      <div
                        className={`w-16 h-16 ${feature.color} rounded-2xl mx-auto mb-4 flex items-center justify-center group-hover:scale-110 transition-transform duration-300`}
                      >
                        <feature.icon className="h-8 w-8 text-white" />
                      </div>
                      <h3
                        className={`${typographyScale.h5} text-white mb-3 group-hover:text-gold-300 transition-colors duration-300`}
                      >
                        {feature.title}
                      </h3>
                      <p className={`${typographyScale.body} text-gray-300`}>
                ...
                {sortedArticles.map((article) => (
                  ...
                  >
                    ...
                    <div className="flex-1">
                      <h4
                        className={`${typographyScale.h5} text-slate-900 mb-1`}
                      >
                        {article.title}
                      </h4>
